...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<Direction>(translator.PushDirection, translator.Get, translator.UpdateDirection);
        ...
        int Direction_TypeID = -1;
		int Direction_EnumRef = -1;
        
        public void PushDirection(RealStatePtr L, Direction val)
        {
            if (Direction_TypeID == -1)
            {
			    bool is_first;
                Direction_TypeID = getTypeId(L, typeof(Direction), out is_first);
				
				if (Direction_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(Direction));
				    Direction_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, Direction_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, Direction_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for Direction ,value="+val);
            }
			
			LuaAPI.lua_getref(L, Direction_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out Direction val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != Direction_TypeID)
				{
				    throw new Exception("invalid userdata for Direction");
				}
                ...
                {
                    throw new Exception("unpack fail for Direction");
                }
				val = (Direction)e;
            ...
            {
                val = (Direction)objectCasters.GetCaster(typeof(Direction))(L, index, null);
            }
        }
		
        public void UpdateDirection(RealStatePtr L, int index, Direction val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for Direction ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			{
			    Direction[] array = obj as Direction[];
				translator.PushDirection(L, array[index]);
				return true;
			}
            return false;
		}
