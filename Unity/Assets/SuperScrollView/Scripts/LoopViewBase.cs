...
namespace NextFramework.UI
{
    ...
    public class LoopViewBase : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        [SerializeField] protected List<ItemPrefabConfData> m_ItemPrefabDataList = new List<ItemPrefabConfData>();
        [SerializeField] ArrangeType m_ArrangeType = ArrangeType.TopToBottom;
        ...
        protected RectTransform mScrollRectTransform = null;
        protected RectTransform mViewPortRectTransform = null;

        protected Dictionary<string, ItemPool> mItemPoolDict = new Dictionary<string, ItemPool>();
        protected List<ItemPool> mItemPoolList = new List<ItemPool>();
        ...
        Func<LoopViewBase, int, LoopListViewItem2> mOnGetItemByIndex;
        ...
        public System.Action<LoopViewBase, LoopListViewItem2> mOnSnapItemFinished = null;
        public System.Action<LoopViewBase, LoopListViewItem2> mOnSnapNearestChanged = null;
        ...
        public ArrangeType ArrangeType { get { return m_ArrangeType; } set { m_ArrangeType = value; } }
        ...
        public virtual ItemPrefabConfData GetItemPrefabConfData(string prefabName)
        {
            foreach (ItemPrefabConfData data in m_ItemPrefabDataList)
            {
        ...
        public void OnItemPrefabChanged(string prefabName)
        {
            ...
            ItemPool pool = null;
            if (!mItemPoolDict.TryGetValue(prefabName, out pool)) return;
        ...
        /*
        InitListView method is to initiate the LoopViewBase component. There are 3 parameters:
        ...
        */
        public void InitListView(int itemTotalCount,
            Func<LoopViewBase, int, LoopListViewItem2> onGetItemByIndex,
            LoopListViewInitParam initParam = null)
        {
            ...
            mIsVertList = (m_ArrangeType == ArrangeType.TopToBottom || m_ArrangeType == ArrangeType.BottomToTop);
            ...
            if (mListViewInited)
            {
                Debug.LogError("LoopViewBase.InitListView method can be called only once.");
            }
        ...
        */
        public virtual void SetListItemCount(int itemCount, bool resetPos = true)
        {
            if (itemCount == mItemTotalCount) return;
        ...
        {
            get { return mIsVertList ? ViewPortHeight : ViewPortWidth; }
        }
        ...
        */
        ...
        {
            ItemPool pool = null;
            if (!mItemPoolDict.TryGetValue(itemPrefabName, out pool)) return null;

            LoopListViewItem2 item = pool.Alloc();
        ...
        /*
        For a vertical scrollrect, when a visible item’s height changed at runtime, then this method should be called to let the LoopViewBase component reposition all visible items’ position.
        For a horizontal scrollrect, when a visible item’s width changed at runtime, then this method should be called to let the LoopViewBase component reposition all visible items’ position.
        */
        ...
        */
        public void MovePanelToItemIndex(int itemIndex, float offset)
        {
            ...
            if (m_ArrangeType == ArrangeType.TopToBottom)
            {
            ...
            else if (m_ArrangeType == ArrangeType.BottomToTop)
            {
                float containerPos = mContainerTrans.localPosition.y;
                if (containerPos > 0)
                {
            ...
            else if (m_ArrangeType == ArrangeType.LeftToRight)
            {
            ...
            else if (m_ArrangeType == ArrangeType.RightToLeft)
            {
                float containerPos = mContainerTrans.localPosition.x;
                if (containerPos < 0)
                {
        ...
        void RecycleItemTmp(LoopListViewItem2 item)
        {
            ...
            ItemPool pool = null;
            if (!mItemPoolDict.TryGetValue(item.ItemPrefabName, out pool)) return;
            pool.Recycle(item);
        ...
        void AdjustContainerPivot(RectTransform rtf)
        {
            ...
            if (m_ArrangeType == ArrangeType.BottomToTop)
            {
                pivot.y = 0;
            }
            else if (m_ArrangeType == ArrangeType.TopToBottom)
            {
                pivot.y = 1;
            }
            else if (m_ArrangeType == ArrangeType.LeftToRight)
        ...
        protected void AdjustPivot(RectTransform rtf)
        {
            Vector2 pivot = rtf.pivot;
        ...
        protected void AdjustAnchor(RectTransform rtf)
        {
            Vector2 anchorMin = rtf.anchorMin;
            Vector2 anchorMax = rtf.anchorMax;
            if (m_ArrangeType == ArrangeType.BottomToTop)
        ...
        protected virtual void InitItemPool()
        {
            foreach (ItemPrefabConfData data in m_ItemPrefabDataList)
            {
                if (data.mItemPrefab == null)
        ...
        {
            if (m_ShowScrollBar && index < 0) return null;
            ...
            if (needCheck)
            {
                ...
                else if (m_ArrangeType == ArrangeType.BottomToTop)
                {
                    ...
                    for (int i = 0; i < count; ++i)
                    {
                        curDist = snapCenter - itemSnapCenter;
                        curDistAbs = Mathf.Abs(curDist);
                        if (curDistAbs >= curMinDist) break;
        ...
        void UpdateSnapHorizontal(bool immediate = false)
        {
            ...
            if (needCheck)
            {
                ...
                float curDistAbs = 0;
                float snapCenter = 0;
                if (m_ArrangeType == ArrangeType.RightToLeft)
                {
                    snapCenter = -(1 - mViewPortSnapPivot.x) * mViewPortRectTransform.rect.width;
                    Vector3 rightPos1 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[2]);
                    start = rightPos1.x;
            ...
            if (m_ArrangeType == ArrangeType.LeftToRight)
            {
        ...
        bool CanSnap()
        {
            ...
            float v = Mathf.Abs(mIsVertList ? mScrollRect.velocity.y : mScrollRect.velocity.x);
