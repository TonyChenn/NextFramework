...
namespace NextFramework
{
    public class WebServer : MonoSinglton<WebServer>
    {
        ...
        {
            get { return PackageConfig.Singlton.GetFtpUrl(); }
        }
        #region Get Data
        /// <summary>
        /// Get
        ...
        {
            GetDataByUrl(PathHelper.CombinePath(Singlton.FtpUrl, path), result);
        }
        IEnumerator GetData(string url, Action<UnityWebRequest> result)
        {
            ...
            {
                string info = string.Format("Get error: {0}, url: {1}", request.error, url);
                Log.Error(info);
                MessageBox.ShowDialog(info);
            }
            else
            {
